using OpenCover.Framework.Model;
using System;
using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using File = System.IO.File;

public class GameSettings : MonoBehaviour
{
    private const string _settingsFileName = "settings.txt";

    private static bool _isMuted;

    public static bool IsMuted
    {
        get => _isMuted;
        set { _isMuted = value; SaveSettings(); }
    }

    private static float _backgroundVolum;

    public static float BackgroundVolum
    {
        get => _backgroundVolum;
        set { _backgroundVolum = value; SaveSettings(); }
    }

    public static void SaveSettings()
    {
        string path = Application.persistentDataPath + "/" + _settingsFileName;

        string data = $"{_isMuted}\n{_backgroundVolum}";

        File.WriteAllText(path, data);
    }

    public static void LoadSettings()
    {
        string path = Application.persistentDataPath + "/" + _settingsFileName;

        if(File.Exists(path))
        {
            string[] lines = File.ReadAllLines(path);

            _isMuted = lines.Length > 0 ? Convert.ToBoolean(lines[0]) : false;
            _backgroundVolum = lines.Length > 0 ? Convert.ToSingle(lines[1]) : 0;
        }
    }
}
